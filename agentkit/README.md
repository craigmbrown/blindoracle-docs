...
- **Delegation chain** — `ProofOfDelegation` (kind 30014), HMAC-signed and hash-chained; emitted **by default** on each agent spawn (disable per service via `BO_DELEGATION_PROOFS_ENABLED=0`), tracing back to the operator and independently verifiable via `verify_delegation_chain` (kind-filtered, so a shared ProofDB stays verifiable)
